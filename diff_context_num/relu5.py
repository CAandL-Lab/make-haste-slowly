...
if __name__ == "__main__":
    ...
    run_idx = 0
    num_hidden = 600 #60 #8.0 #20.0 (for GDLN work using random weights and compression to share latent)
    layer_sizes_relu = [(num_obj+5), int(num_hidden), (2*num_obj-1)*6]
    ...
    for epoch in range(num_epochs):
        ...
        if epoch in [2000]:
            plot_outputs(predict_relu(params_relu, X), new_cmap, 'plots/relu5_out'+str(epoch)+'.pdf', vmin=-1, vmax=1)
        
    # Num epoch, num_data, hidden_dim
    np.savetxt('losses/relu5.txt', losses)
