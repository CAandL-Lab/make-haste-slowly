...
def predict_gated_context(params, inputs):
  ...
  # Apply module a -> context 1,2,3
  m1 = jnp.dot(params[1][:,num_hidden*1:num_hidden*2],h1[num_hidden*1:num_hidden*2,num_obj*0:num_obj*3])
  out = out.at[:,num_obj*0:num_obj*3].add(m1)
  # Apply module b -> context 2,3,4
  m2 = jnp.dot(params[1][:,num_hidden*2:num_hidden*3],h1[num_hidden*2:num_hidden*3,num_obj*1:num_obj*4])
  out = out.at[:,num_obj*1:num_obj*4].add(m2)
  # Apply module c -> context 1,3,4
  ...
  m4 = jnp.dot(params[1][:,num_hidden*3:num_hidden*4],h1[num_hidden*3:num_hidden*4,num_obj*2:num_obj*4])
  out = out.at[:,num_obj*2:num_obj*4].add(m4)
  # Apply module d -> context 1,2,4
  ...
  m6 = jnp.dot(params[1][:,num_hidden*4:num_hidden*5],h1[num_hidden*4:num_hidden*5,num_obj*3:num_obj*4])
  out = out.at[:,num_obj*3:num_obj*4].add(m6)
  return out
...
def predict_gated_context_modules(params, inputs):
  ...
  m6 = jnp.dot(params[1][:,num_hidden*4:num_hidden*5],h1[num_hidden*4:num_hidden*5,num_obj*3:num_obj*4])
  out = out.at[:,num_obj*3:num_obj*4].add(m6)
  return [m1,m2,jnp.hstack([m3,m4]),jnp.hstack([m5,m6])]
...
if __name__ == "__main__":
    ...
    layer_sizes_gated = [num_obj+4, int(num_hidden), (2*num_obj-1)*5]
    ...
    param_scale_gated = [ [[1e1*param_scale_base, 1e1*param_scale_base, 1e1*param_scale_base, 1e1*param_scale_base,1e1*param_scale_base]] ,
                        [[1e1*param_scale_base], [1e1*param_scale_base], [1e1*param_scale_base], [1e1*param_scale_base],[1e1*param_scale_base]] ]
    ...
    shared_svs = np.zeros( (num_epochs, num_obj+4) )
    svs_d = np.zeros( (num_epochs, num_obj+4) )
    svs_e = np.zeros( (num_epochs, num_obj+4) )
    svs_f = np.zeros( (num_epochs, num_obj+4) )
    svs_g = np.zeros( (num_epochs, num_obj+4) )
    ...
    for epoch in range(num_epochs):
        ...
        if epoch in [2000]:
            plot_outputs(predict_gated_context_modules(params_gated, X[:num_obj])[0], new_cmap, 'plots/gdln4_cont_out'+str(epoch)+'.pdf', vmin=-1, vmax=1)
            plot_outputs(predict_gated(params_gated, X), new_cmap, 'plots/gdln4_out'+str(epoch)+'.pdf', vmin=-1, vmax=1)
    ...
    np.savetxt('losses/gated4.txt', losses)
    np.savetxt('svs4/gated4_shared.txt', shared_svs)
    np.savetxt('svs4/gated4_d.txt', svs_d)
    np.savetxt('svs4/gated4_e.txt', svs_e)
    np.savetxt('svs4/gated4_f.txt', svs_f)
    np.savetxt('svs4/gated4_g.txt', svs_g)
